  ...
  <div class="register-view">
    ...
    <div class="form-group">
      ...
      <select v-model="form.position" class="form-input">
        ...
        <option value="grade_director">年级主任</option>
      </select>
    </div>
...
const form = reactive({
  position: '', realName: '', password: '', confirmPassword: '',
  subject: '', phone: '', verifyCode: ''
})
...
async function handleRegister() {
  ...
  if (!form.phone || !form.password || !form.realName || !form.subject) {
    alert('请填写必填项'); return
  }
  loading.value = true
  try {
    await auth.register({ ...form, role: 'teacher' })
    alert('注册成功，请登录')
