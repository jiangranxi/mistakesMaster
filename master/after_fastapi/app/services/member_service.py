...
class MemberService:
    ...
    async def get_orders(self, user_id: uuid.UUID, page: int, page_size: int, status: str) -> dict:
        items, total = await self.order_repo.get_by_user(user_id, page, page_size, status)
        return {
            "list": [
                {
                    "id": str(o.id),
